...
- [Checkpoints must be able to decide](feedback-checkpoint-must-be-able-to-decide.md) — before shipping a checkpoint rule, check the effect it targets is bigger than the measurement noise over its window, and re-derive its threshold from the *live* target rather than an inherited design rate (root of the Aug 11 re-read being unable to test what it was built for)
