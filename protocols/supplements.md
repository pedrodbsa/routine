# SUPPLEMENT PROTOCOL - MASTER

> Core stack is deliberately conservative. All listed items are compatible with finasteride 1 mg/day.

## Daily Schedule

> Timing updated 2026-05-31 off the retired intermittent-fasting window — breakfast is now eaten 07:00-09:00 every day, so "first meal" supplements move to breakfast.

### Breakfast (07:00-09:00)

- Vitamin D3 + K2: 4,000-6,000 IU D3 / 100-120 mcg K2
- Multivitamin: 1 tablet

### Creatine Monohydrate — 5 g, one dose/day

Timing is irrelevant for creatine. What matters is that the dose happens **every day, including rest days and travel days**.

> ⚠ **Do not attach the dose to the post-workout shake (corrected 2026-07-29).** The old instruction — "take it with whichever shake is convenient" — coupled a daily supplement to a feed that is not daily. The shake gets dropped or shrunk routinely (Jun 25, Jul 20, Jul 22, Jul 27, Jul 28 all logged), and each time the dose survived only if the athlete remembered to relocate it. **Athlete reports intake fell away from the start of the Jul 6 holiday — 23 days by 2026-07-29.** This is the same failure mode as the isotretinoin dose attached to dinner: a daily thing hung on a non-daily anchor.
>
> **Attach it to a fixed anchor that exists on all seven days** — the tub sits next to the coffee, and the dose goes in the first drink of the day regardless of what training happens later. On days a shake *is* taken, that is fine too, but the shake is no longer the carrier of record.

**Re-saturation note (relevant 2026-07-29 → ~late August).** Restarting 5 g/day after a ~3-week gap will put **0.5–1.5 kg back on the scale over 2–4 weeks**. That is intracellular water in muscle, not fat, and it is the desired effect. Do **not** read it as a stall or a regain, and do not respond to it with a calorie cut. It will also lift the scale's BIA "muscle mass" channel back up for the same reason.

**Why it matters more, not less, during a cut.** Creatine is the one supplement that clears the athlete's own evidence bar, and its value rises under a deficit where lean-mass retention is a stated priority. Dropping it during Cut Block 2 is the wrong direction.

### Lunch (~13:00)

- Omega-3 Fish Oil: 2 g combined EPA/DHA
- Zinc: 20 mg
- Finasteride: 1 mg

### Pre-Bed (~22:00)

- Magnesium glycinate: 200-350 mg elemental (kept ≤ the 350 mg supplemental UL; the daily food target already supplies ~400 mg). Well-tolerated and corrects a common dietary shortfall; sleep-onset benefit is modest/uncertain — not a primary sleep lever.

## Strategic / Optional

- Caffeine: 200-250 mg before quality sessions or races. **Hard cutoff ~10:00-11:00 on normal days** — tightened from 13:00 because sleep-onset latency is the primary recovery constraint and caffeine's ~5-6 h half-life leaves a meaningful fraction circulating at a 22:00-23:00 bedtime. If a quality session is in the afternoon, that day's caffeine is the pre-session dose — no separate morning cup on top.
- Collagen Peptides: 15 g + ~50 mg vitamin C 45-60 min before running during high-volume blocks or if tendon stiffness appears
- Astaxanthin: not recommended as a daily supplement during aerobic-development blocks; only consider it temporarily for a specific non-performance reason

## As-Needed Only

- Curcumin 1,000 mg with piperine only for pain/inflammation >48 h. Do not use daily during training blocks.

## Compatibility Notes

- Creatine: safe with finasteride
- Vitamin D3 + K2: safe
- Omega-3: safe
- Magnesium: safe
- Zinc: safe if total daily intake stays below the tolerable upper limit
- Curcumin: use sparingly; not a direct finasteride problem, but chronic anti-inflammatory use can blunt adaptation

## Removed / Do Not Readd Automatically

| Supplement         | Reason                                                      | Revisit                                |
| ------------------ | ----------------------------------------------------------- | -------------------------------------- |
| B12 + Folate       | Bloodwork showed B12 elevated; multivitamin covers baseline | Next bloodwork                         |
| Ashwagandha KSM-66 | Inconsistent use and unclear need                           | If stress profile changes materially   |
| L-Theanine         | Inconsistent use; sleep-onset evidence is weak              | Onset issues are now recurrent, but kept OFF by athlete preference (supplements only if heavily backed, e.g. creatine). The caffeine cutoff and stimulus control carry the onset work instead. |
